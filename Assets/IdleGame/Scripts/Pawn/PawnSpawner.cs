...
public class PawnSpawner : MonoBehaviour
{
    ...
    {
        Pawn characterPawn = Instantiate(_character, _characterTransform.transform.position, Quaternion.identity,parent.transform);
        //Vector3 charactertransform = new Vector3(0, 0, 0);
        characterPawn.transform.localPosition = _characterTransform.transform.localPosition;
    ...
    private void SpawnEnemyPawn()
    {
        ...
        for (int i = 0; i < _enemyPool.Length; i++)
        {
            ...
            {
                Pawn enemyPawn = Instantiate(_enemyPool[i], _enemyTransform.transform.position, Quaternion.identity, parent.transform);
                enemyPawn.transform.localPosition = _enemyTransform.transform.localPosition;
            ...
            for (int i = 0; i < _scenePawnList.Count; i++)
            {
                if (_scenePawnList[i].PawnConfiguration.ConfigurationType == pawn.PawnConfiguration.ConfigurationType)
                {
                    ...
                    for (int j = 0; j < _enemyPool.Length; j++)
                    {
                        ...
                        {
                            Pawn enemyPawn = Instantiate(_enemyPool[j], _enemyTransform.transform.position, Quaternion.identity,parent.transform);
                            enemyPawn.transform.localPosition = _enemyTransform.transform.localPosition;
                            enemyPawn.transform.rotation = parent.transform.rotation;
                            _scenePawnList.Add(enemyPawn);
                            break;
                        }
                    }
